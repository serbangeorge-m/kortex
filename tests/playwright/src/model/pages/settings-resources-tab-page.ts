...
export class SettingsResourcesPage extends BasePage {
  ...
  async openCreateMilvusPage(): Promise<SettingsCreateMilvusPage> {
    return this.openTab(this.getResourceCreateButton(resources.milvus.displayName), SettingsCreateMilvusPage);
  }
  ...
  getCreatedConnectionsFor(resourceId: keyof typeof resources, connectionType: ConnectionType = 'inference'): Locator {
    ...
    const providerRegion =
      connectionType === 'rag' ? this.getProviderRegion(resourceId) : this.getResourceRegion(resourceId);
    return providerRegion.getByRole('region').filter({ hasText: label });
  }
