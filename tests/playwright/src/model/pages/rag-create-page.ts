...
export class RagCreatePage extends BasePage {
  ...
  readonly vectorStoreSection: Locator;
  readonly embeddingModelSection: Locator;
  readonly createButton: Locator;
  readonly cancelButton: Locator;

  constructor(page: Page) {
    super(page);
    this.modal = this.page.locator('[aria-label="create knowledge environment"]');
    this.heading = this.modal.getByRole('heading', { name: 'New Knowledge Environment' });
    this.nameInputField = this.modal.locator('#environmentName');
    this.vectorStoreSection = this.modal.locator('.flex.flex-col.space-y-3').filter({ hasText: 'Vector Store' });
    this.embeddingModelSection = this.modal.locator('.flex.flex-col.space-y-3').filter({ hasText: 'Embedding Model' });
  ...
  getVectorStoreTile(name: string): Locator {
    return this.vectorStoreSection.locator('.tile').filter({ hasText: name });
  }

  getEmbeddingModelTile(name: string): Locator {
    return this.embeddingModelSection.locator('.tile').filter({ hasText: name });
  }
