...
vi.mock(import('./ollama-extension'));
...
test('should initialize and activate the OllamaExtension when activate is called', async () => {
  ...
  // Ensure that the OllamaExtension is instantiated and its activate method is called
  expect(OllamaExtension.prototype.activate).toHaveBeenCalled();
});

test('should call deactivate when deactivate is called', async () => {
  // Call activate first to initialize ollamaExtension
  await activate(extensionContextMock);
